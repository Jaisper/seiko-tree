...
The whole project is one `index.html` file. Inside the `<script>` block at the bottom, the code is divided into ten clearly-marked sections — jump straight to the one you need to edit.
...
SECTION  5 · NODE POSITIONS           — x/y coordinates and visual properties
SECTION  6 · RELATIONSHIPS            — edges (lines between nodes)
SECTION  7 · CONTAINER BOXES          — sections (labelled rectangles)
SECTION  8 · FILTER TAGS              — nodeTags (for category filtering)
SECTION  9 · GUIDED TOURS             — narrated walk-throughs
SECTION 10 · RENDERING & INTERACTION  — the engine (usually no edits needed)
```

Within Sections 4, 5 and 6, every watch is grouped into a clearly-labelled family. For example:

```js
/* ─── SUWA · MARVEL & CROWN FOUNDATION ─── */
/* The roots of Seiko mechanical watchmaking at Suwa Seikosha. Marvel (1956)
   was Seiko's first fully in-house movement; Crown (1959) became its
   chronometer-grade evolution and led directly to the first Grand Seiko. */

marvel: {n:"Seiko Marvel", era:"1956-62", d:"...", tags:["icon"]},
gyro_marvel: {n:"Gyro Marvel", era:"1959-63", d:"...", tags:[]},
lord_marvel: {n:"Lord Marvel", era:"late 1950s-75", d:"...", tags:[]},
...
```

Each watch lives on its own line. Open `index.html` in any text editor, hit Ctrl/Cmd-F to find the watch you want, and edit it.
...
- Section 5 (`const nodes`): fix the visible label on the box (the `\n` is a line break)
...
2. In Section 4, add an entry to `popupData` in the right family group:
   ...
   ```
3. In Section 5, add a node entry with coordinates:
   ```js
   ["ks_special_72", "KS Special\n1972", 850, 1640, C.ks, 0],
   ```
   `x` and `y` are pixel coordinates on a canvas that is roughly 2500×2200. Standard horizontal stride between adjacent nodes is 160px, vertical stride is 65px.
4. In Section 6, optionally add edges linking it to other watches:
   ```js
...
**Add a guided tour**
Go to Section 9 (`const tours`) and add an entry following the format of the existing ones. Each stop is a node id plus a narration paragraph.
...
- ~170 KB total (one file, includes everything)
...
This project is the work of an enthusiast, not a Seiko employee. It owes everything to the long tradition of Seiko collectors, archivists, and writers who have done the real research over decades. Thanks especially to the wider Seiko community, Plus9Time and GS9 Club archives, and Seiko's own design-history publications.
